...
*/
    ...
    method RSA_e_3_verify(key: pub_key, signature: seq<uint32>, sha: seq<uint32>)
        returns (x : bool)
        
        requires valid_pub_key(key);
        requires |signature| == |sha| == key.len;
        requires 0 <= seq_interp(signature) < key.m_val; 
    {
